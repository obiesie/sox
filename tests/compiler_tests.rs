...
static ALL_TEST_SUITES: [&str; 13] = ["assignment", "block", "bool", "call", "class", "closure", "for", 
    "function", "if", "number", "operator", "print", "while"];
...
fn test_compiler(){
    ...
    let test_suites = if TEST_SUITES.is_empty() { ALL_TEST_SUITES.to_vec() } else { TEST_SUITES.to_vec()};
